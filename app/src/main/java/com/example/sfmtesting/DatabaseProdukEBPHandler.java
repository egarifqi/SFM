...
public class DatabaseProdukEBPHandler extends SQLiteOpenHelper {
    private static final int DATABASE_VERSION = 1;
    private static final String DATABASE_NAME = "produkEBPManager";
    private static final String TABLE_PRODUK_EBP = "produkEBP";
    ...
    public DatabaseProdukEBPHandler(Context context) {
    ...
    public void onCreate(SQLiteDatabase db) {
        String CREATE_CONTACTS_TABLE = "CREATE TABLE " + TABLE_PRODUK_EBP + "("
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_PRODUK_EBP);
...
//        ContentValues values = new ContentValues();
        ...
        // Inserting Row
        db.insert(TABLE_PRODUK_EBP, null, values);
...
//        Log.e("DATABASE", db.toString());
    ...
    com.example.sfmtesting.Spacecraft getProduk(int id) {
        ...
        Cursor cursor = db.query(TABLE_PRODUK_EBP, new String[] { KEY_ID, KEY_CODE, KEY_NAME, KEY_PRICE,
                        KEY_CATEGORY, KEY_BARCODE, KEY_WEEKLY_SALES,
    ...
    com.example.sfmtesting.Spacecraft getProdukToko(int id, String partnerid, String brand) {
        SQLiteDatabase db = this.getReadableDatabase();
        com.example.sfmtesting.Spacecraft s = new com.example.sfmtesting.Spacecraft();
    ...
    public ArrayList<com.example.sfmtesting.Spacecraft> getAllProduk() {
        ...
        // Select All Query
        String selectQuery = "SELECT * FROM " + TABLE_PRODUK_EBP;
...
//                Spacecraft contact = new Spacecraft();
                        ...
                        contact.setWeeklySales(cursor.getInt(6));
                        contact.setPartner_id(cursor.getString(7));
                        contact.setBrand(cursor.getString(8));
                        contact.setStock(cursor.getString(9));
                        contact.setQty(cursor.getString(10));
                        contact.setKoli(cursor.getString(11));
...
//        Spacecraft contact = new Spacecraft();
        // Select All Query
        String selectQuery = "SELECT * FROM " + TABLE_PRODUK_EBP + " WHERE " + KEY_PARTNER_ID + " IS " + partnerid + " AND " + KEY_BRAND + " IS '" + brand + "'";
...
//        ContentValues values = new ContentValues();
        ...
        // updating row
        return db.update(TABLE_PRODUK_EBP, values, KEY_ID + " = ?",
    ...
    public void deleteProduk(com.example.sfmtesting.Spacecraft contact) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(TABLE_PRODUK_EBP, KEY_ID + " = ?",
    ...
    public void deleteAll(){
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "DELETE FROM " + TABLE_PRODUK_EBP;
        db.execSQL(query);
        db.close();
    }
    ...
    public int getProductCount() {
        String countQuery = "SELECT  * FROM " + TABLE_PRODUK_EBP;
        SQLiteDatabase db = this.getReadableDatabase();
